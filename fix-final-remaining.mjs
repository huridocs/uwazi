...
async function run() {
  console.log('🔍 Finding and fixing final remaining syntax errors...');
  ...
  for (const file of files) {
    try {
      ...
      const { content: modifiedContent, changed } = fixFinalRemaining(content);
      
      if (changed) {
        fs.writeFileSync(file, modifiedContent);
        console.log(`  ✅ Fixed final remaining syntax in ${file}`);
        filesModified++;
        totalReplacements++;
      }
    } catch (error) {
