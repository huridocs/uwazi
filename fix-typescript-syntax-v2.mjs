...
function fixTypeScriptSyntax(content) {
  ...
  // Pattern: .catch(param: any) => should be .catch((param: any) =>
  modifiedContent = modifiedContent.replace(
    /\.catch\((\w+):\s*any\)\s*=>\s*{/g,
    (match, param) => {
      changed = true;
  ...
  // Pattern: ): ReturnType => should be ): ReturnType =>
  modifiedContent = modifiedContent.replace(
    /\):\s*(\w+)\s*=>\s*{/g,
    (match, returnType) => {
      changed = true;
  ...
  // Fix incorrect parameter type annotations in nested calls
  // Pattern: (param: any) => should be (param: any) =>
  modifiedContent = modifiedContent.replace(
    /\((\w+):\s*any\)\s*=>\s*{/g,
    (match, param) => {
...
async function run() {
  console.log('🔍 Finding and fixing remaining TypeScript syntax errors...');
