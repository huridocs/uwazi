...
// Dynamic path alias conversion function
...
async function fixPathAliases() {
  console.log('🔍 Finding files with path aliases...');
  
  // Find all TypeScript and JavaScript files
  const files = await glob('app/**/*.{ts,js,tsx}', { 
    cwd: process.cwd(),
    ignore: ['app/dist/**', 'app/node_modules/**', '**/*.spec.*', '**/*.test.*']
  });
  ...
  for (const file of files) {
    try {
      ...
      // Check for path aliases using regex
      ...
      // Replace path aliases dynamically
...
fixPathAliases().catch(console.error);
