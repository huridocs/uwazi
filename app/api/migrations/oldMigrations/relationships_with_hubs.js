...
function sanitizeExistingData() {
  ...
  .then(relations => P.resolve(relations).map((relationship) => {
...
function migrateTemplates() {
  ...
  .then(_templates => P.resolve(_templates).map(template => createRelationTypesWhenNotExists(template.properties)
  .then(() => {
  ...
  .then(templateToSave => templatesModel.save(templateToSave)).catch(console.log), { concurrency: 1 }).catch(console.log));
}

let relationsProcessed = 0;
let totalRelations;
const relationsProcessedInAdifferentHub = [];
  ...
  .then(([relations, _settings]) => {
    ...
    return P.resolve(relations).map((relationship) => {
      ...
      if (relationship.sourceType === 'metadata') {
        ...
        .then(([relationType]) => Promise.all([Promise.resolve(relationType), relationshipsModel.get({
      ...
      return Promise.all([
        ...
        relationships.save({
          hub,
          entity: relationship.targetDocument,
          range: relationship.targetRange,
          template: relationship.relationType,
...
function migrateEntities() {
  ...
  .then(([_entities, _templates]) => {
    ...
    const keyTemplates = _templates.reduce((response, template) => {
    ...
    .then((entity) => {
      ...
      const template = keyTemplates[entity.template];
