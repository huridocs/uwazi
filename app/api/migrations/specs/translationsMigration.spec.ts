...
const alreadyInAllContexts: { [key: string]: string } = {
  key: 'Upload PDF',
  en: 'Upload PDF',
  es: 'Subir PDF',
  hu: 'PDF Feltöltése',
...
describe('migration add_system_key_translations', () => {
  ...
  it('should append new keys, leave existing keys intact.', async () => {
    await migrateTranslationsFromCSV(
      testingDB.mongodb!,
      'app/api/migrations/specs/system_keys.csv'
    );
    const allTranslations = await testingDB
      .mongodb!.collection('translations')
      .find()
      .toArray();
    function testKeyValue(key: string, value: string, locale: string, contextId: string) {
      expect(
        allTranslations
          .find(tr => tr.locale === locale)
          .contexts.find((c: TranslationContext) => c.id === contextId)
          .values.find((v: any) => v.key === key).value
      ).toBe(value);
    }
    ...
    locales.forEach(loc => {
      expect(
        allTranslations
          ...
          .contexts.find((c: TranslationContext) => c.id === templateId.toString()).values
      ).toHaveLength(2);
