...
describe('CsvCleanupImportFilesJob (integration)', () => {
  ...
  const setUp = () => {
    ...
    return CsvCleanupImportFilesJobFactory.build({ fileStorage });
  };
  ...
  describe.each(csvJobBackendConfigs)('$name', ({ postgresCsv, postgresCore }) => {
    ...
    itWithContext('should remove csv import artifacts and mark filesCleanup done', async () => {
      ...
      await expect(
        fs.access(
          pathManager.createPath({ type: 'customPath', destination, filename: 'upload.zip' })
        )
      ).rejects.toBeDefined();
      await expect(
        fs.access(
    ...
    itWithContext('should not run cleanup for non-terminal imports', async () => {
      const { useCase, csvImportsDS, fileStorage } = setUp();
      const f = getFixturesFactory();
      ...
      await expect(
        fs.access(
          pathManager.createPath({ type: 'customPath', destination, filename: 'upload.csv' })
        )
      ).resolves.toBeUndefined();
    });

    itWithContext('should be idempotent when files are already missing', async () => {
