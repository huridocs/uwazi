...
describe('CsvExtractUploadedZipJob (integration)', () => {
  ...
  describe.each(csvJobBackendConfigs)('$name', ({ postgresCsv, postgresCore }) => {
    ...
    itWithContext(
      'should normalize a single CSV to extracted/import.csv, stage rows, and set final status',
      async () => {
    ...
    itWithContext('should stage empty CSV rows so indexes match the source file', async () => {
    ...
    itWithContext('should stage rows in batches for large CSVs', async () => {
    ...
    itWithContext(
      'should extract a ZIP with root files, stage rows, and require import.csv',
      async () => {
        ...
        const csvContent = [
          'title,description',
          'Zip Row 1,Zip Value 1',
          'Zip Row 2,Zip Value 2',
        ].join('\n');
    ...
    itWithContext(
      'should throw when import not found (??? This originally was a NonRetryableJobError)',
      async () => {
        const { useCase } = setUp();
        const f = getFixturesFactory();
        await expect(
          useCase.execute(buildInput(f.idString('non-existent')))
        ).rejects.toBeInstanceOf(CsvImportDoesNotExistError);
      }
    );

    itWithContext('should throw NonRetryableJobError when storage path is missing', async () => {
    ...
    itWithContext('should set failed when ZIP is missing import.csv', async () => {
