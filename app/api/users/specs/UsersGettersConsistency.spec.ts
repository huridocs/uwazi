...
/**
 * Pins the two v1 getters in `users.js` that still have callers. It used to run twice, once
 * per setting of the flag that decided whether `/api/users` reached them at all; that flag is
 * gone and these getters never read it themselves, so one run says the same thing. Delete
 * this file along with the getters it covers.
 */
describe('v1 users getters', () => {
  beforeAll(async () => {
  ...
  describe('get()', () => {
    ...
    it('should exclude soft-deleted users', async () => {
      ...
      const result = await users.get({});
      expect(result.find((u: any) => u._id.toString() === userToDelete.toString())).toBeUndefined();
    });

    it('should not include groups when they are not selected', async () => {
      const result = await users.get({});
