...
describe('EntityPermissions', () => {
  ...
  describe('allowsPublicReads()', () => {
    it('return show whether the entity is published or not', () => {
      const publishedPermissions = new EntityPermissions('entity', [], true);
      expect(publishedPermissions.allowsPublicReads()).toBe(true);
      const unpublishedPermissions = new EntityPermissions('entity', [], false);
      expect(unpublishedPermissions.allowsPublicReads()).toBe(false);
    });
  });
});
