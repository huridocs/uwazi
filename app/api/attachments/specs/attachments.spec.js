...
describe('attachments', () => {
  ...
  describe('/delete', () => {
    beforeEach(async () => {
      await fs.writeFile(path.join(paths.attachmentsPath, 'attachment.txt'), 'dummy file');
      await fs.writeFile(path.join(paths.attachmentsPath, 'mainFile.txt'), 'dummy file');
      await fs.writeFile(path.join(paths.attachmentsPath, `${toDeleteId.toString()}.jpg`), 'dummy file');
      await fs.writeFile(path.join(paths.attachmentsPath, `${entityId.toString()}.jpg`), 'dummy file');
      await fs.writeFile(path.join(paths.attachmentsPath, `${entityIdEn.toString()}.jpg`), 'dummy file');
    ...
    it('should remove main file and thumbnail if id matches entity', async () => {
      ...
      expect(await fs.exists(path.join(paths.attachmentsPath, 'mainFile.txt'))).toBe(false);
      expect(await fs.exists(path.join(paths.attachmentsPath, `${toDeleteId.toString()}.jpg`))).toBe(false);
    });
    ...
    it('should remove main file on sibling entities', async () => {
      ...
      changedEntities.forEach(async (e) => {
        ...
        expect(await fs.exists(path.join(paths.attachmentsPath, `${e._id.toString()}.jpg`))).toBe(false);
      });
    });
    ...
    it('should remove the passed file from attachments and delte the local file', async () => {
      ...
      expect(await fs.exists(path.join(paths.attachmentsPath, 'attachment.txt'))).toBe(false);
    });
    ...
    it('should not fail if, for some reason, file doesnt exist', async () => {
      expect(await fs.exists(`${paths.attachmentsPath}attachment.txt`)).toBe(true);
      await fs.unlink(`${paths.attachmentsPath}attachment.txt`);
