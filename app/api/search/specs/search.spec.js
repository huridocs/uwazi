  ...
  describe('search', () => {
    ...
    it('should perform a fullTextSearch on fullText and title', (done) => {
      ...
      .then(([spanish, none, english, batmanFinishes, batmanBegins, batmanOR, batman, fullTextNormal, fullTextExactMatch]) => {
        expect(english.rows.find(r => r.snippets.fullText[0].text.match('<b>english</b> document <b>english</b>')).snippets.fullText[0].page).toBe(12);
        expect(english.rows.find(r => r.snippets.fullText[0].text.match('<b>english</b> another')).snippets.fullText[0].page).toBe(2);
