...
import { documentQueryBuilder, elastic } from 'api/search';
...
describe('search', () => {
    ...
    it('should filter by metadata, and return template aggregations based on the filter the language and the published status', done => {
      Promise.all([
        ...
        .then(([joker, unpublished]) => {
          ...
          expect(
            unpublishedAggs.find(a => a.key === ids.templateMetadata1).filtered.doc_count
          ).toBe(1);
          expect(
            unpublishedAggs.find(a => a.key === ids.templateMetadata2).filtered.doc_count
          ).toBe(0);
          done();
        })
        .catch(catchErrors(done));
        ...
        it('should restrict the results to those who have all values of the filter', done => {
          search
            ...
            .then(filtered => {
              ...
              expect(
                templateAggs.find(a => a.key === ids.templateMetadata1).filtered.doc_count
              ).toBe(1);
  ...
  describe('indexEntities', () => {
    it('should index entities based on query params passed', async () => {
      spyOn(search, 'bulkIndex').and.returnValue(Promise.resolve());
      await search.indexEntities({ sharedId: 'shared' });
    ...
    it('should index entities withh fullText', async () => {
      spyOn(search, 'bulkIndex').and.returnValue(Promise.resolve());
      await search.indexEntities({ sharedId: 'shared' }, '+fullText');
