...
class MongoDatavizQueryExecutor extends MongoDataSource<EntityDBO> implements DatavizQueryExecutor {
  ...
  /**
   * Dataviz aggregations include unpublished entities by default so live queries
   * match snapshots generated by privileged actors. Charts never return
   * individual entities, so viewer permissions do not apply unless the query
   * explicitly opts out with `includeUnpublished: false`.
   */
  private buildPermissionMatch(actor: User, includeUnpublished?: boolean): object {
    if (includeUnpublished !== false || actor.isPrivileged()) {
      return {};
    }
