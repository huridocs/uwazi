...
import { IdFilterCriteriaNode } from 'api/relationships.v2/model/FilterOperatorNodes';
        ...
        template.properties.map(async property => {
          if (
            ...
            const results = await this.relationshipsDataSource
              .getByQuery(
                new MatchQueryNode(new IdFilterCriteriaNode(entity.sharedId), property.query),
                entity.language
              )
              .all();
