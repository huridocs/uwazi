...
const createSut = () => {
  ...
  const jobDispatcher = DefaultDispatcher(tenants.current().name, transactionManager);
  ...
  jest.spyOn(jobDispatcher, 'dispatch').mockResolvedValue();
  ...
  return { sut, fileService, eventBus, transactionManager, dispatcher: jobDispatcher };
};
