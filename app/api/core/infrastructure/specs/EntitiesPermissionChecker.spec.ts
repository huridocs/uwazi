...
describe('EntitiesPermissionChecker', () => {
  beforeAll(async () => {
  ...
  describe.each(backends)('$name backend', ({ usePostgres }) => {
    ...
    const createSut = () =>
      testingEnvironment.runWithContext(() => EntityPermissionCheckerFactory.default());
    ...
    describe('checkReadPermission()', () => {
      it('admin: returns true for unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_2', adminUser)).toBe(true);
      });

      it('editor: returns true for unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_2', editorUser)).toBe(true);
      });

      it('anonymous: returns true for published entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_5', anonymousUser)).toBe(true);
      });

      it('anonymous: returns false for unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_2', anonymousUser)).toBe(false);
      });

      it('collaborator with direct user permission: returns true on unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_3', collaboratorUser)).toBe(true);
      });

      it('collaborator with group permission: returns true on unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_4', collaboratorUser)).toBe(true);
      });

      it('collaborator without permission: returns false on unpublished entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_2', collaboratorUser)).toBe(false);
      });

      it('collaborator without permission: can read published entities', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('entity_5', otherCollaborator)).toBe(true);
      });

      it('non-existent sharedId: returns false (not an error)', async () => {
        const sut = createSut();
        expect(await sut.checkReadPermission('does_not_exist', adminUser)).toBe(false);
      });
    });
    ...
    describe('checkWritePermission()', () => {
      describe('anonymous user', () => {
        it('is always denied', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(entityFile('entity_1'), anonymousUser)).toBe(false);
        });
      });
      ...
      describe('admin user', () => {
        it('is allowed on entity files', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(entityFile('entity_1'), adminUser)).toBe(true);
        });

        it('is allowed on custom (non-entity) files', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(customFile(), adminUser)).toBe(true);
        });
      });
      ...
      describe('editor user', () => {
        it('is denied on custom (non-entity) files', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(customFile(), editorUser)).toBe(false);
        });

        it('is denied on entity files when the editor has no explicit permission', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(entityFile('entity_2'), editorUser)).toBe(false);
        });

        it('is allowed on entity files when the editor has explicit write permission', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('entity_write_editor'), editorUser)
          ).toBe(true);
        });

        it('is denied on entity files when the editor has only read-level permission', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(entityFile('entity_read_only'), editorUser)).toBe(
            false
          );
        });
      });
      ...
      describe('collaborator user', () => {
        it('is denied on custom (non-entity) files', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(customFile(), collaboratorUser)).toBe(false);
        });

        it('is denied on entity files with no permissions', async () => {
          const sut = createSut();
          expect(await sut.checkWritePermission(entityFile('entity_2'), collaboratorUser)).toBe(
            false
          );
        });
        ...
        it('is denied on entity files where the user has only read-level permission', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('entity_read_only'), collaboratorUser)
          ).toBe(false);
        });
        ...
        it('is allowed on entity files where the user has explicit write permission (direct)', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('entity_write_collab'), collaboratorUser)
          ).toBe(true);
        });
        ...
        it('is allowed on entity files where the user belongs to a group with write permission', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('entity_write_group'), collaboratorUser)
          ).toBe(true);
        });
        ...
        it('other collaborator (not in permissions) is denied', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('entity_write_collab'), otherCollaborator)
          ).toBe(false);
        });
      });
      ...
      describe('entity not found in DB', () => {
        it('returns false instead of throwing', async () => {
          const sut = createSut();
          expect(
            await sut.checkWritePermission(entityFile('does_not_exist'), collaboratorUser)
          ).toBe(false);
        });
      });
    });
