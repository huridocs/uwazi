...
const createEntityWithIds = (sharedId: string, languages: string[], template: Template) => {
  ...
  describe.each(backends)('$name backend', ({ usePostgres }) => {
    ...
    const createSut = (accessContext?: AccessContext) =>
      testingEnvironment.runWithContext(() => {
        const sut = EntitiesDataSourceFactory.default(
    ...
    describe('bulkUpdate', () => {
      it('should not overwrite permissions or published when updating entity content', async () => {
        ...
        const entity = createEntityWithIds('bulk-update-entity', ['en', 'es'], template);
        ...
        expect(entityDocs).toMatchObject([
