...
type SeedRow = { _id: string; tenant_id: string; text: string; expiresAt: string };

const insertRows = async (rows: SeedRow[]) => {
  for (const row of rows) {
    // eslint-disable-next-line no-await-in-loop
    await testingPG.pool!.query(
      'INSERT INTO captchas ("_id", "tenant_id", "text", "expiresAt") VALUES ($1, $2, $3, $4)',
      [row._id, row.tenant_id, row.text, row.expiresAt]
    );
  }
};
...
const makeJob = (pool: { query: jest.Mock } | typeof testingPG.pool) => {
  ...
  const job = new CleanupExpiredCaptchasJob({ pool: pool as any, jobsDispatcher });
...
describe('CleanupExpiredCaptchasJob', () => {
  it('should delete expired rows across all tenants in a single run, leaving valid ones', async () => {
    ...
    await insertRows([
      { _id: 'a-expired', tenant_id: TENANT_A, text: 'a1', expiresAt: past },
      { _id: 'a-valid', tenant_id: TENANT_A, text: 'a2', expiresAt: future },
      { _id: 'b-expired', tenant_id: TENANT_B, text: 'b1', expiresAt: past },
      { _id: 'b-valid', tenant_id: TENANT_B, text: 'b2', expiresAt: future },
    ]);

    const { job } = makeJob(testingPG.pool!);
    await job.handleDispatch(noopHeartbeat);

    const remaining = await testingPG.getAllFrom<{ _id: string }>('captchas');
  ...
  it('should re-dispatch itself for the next run after a successful cleanup', async () => {
    ...
    expect(dispatchedClass).toBe(CleanupExpiredCaptchasJob);
