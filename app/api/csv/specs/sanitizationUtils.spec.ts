...
describe('sanitizeStringValue', () => {
  ...
  it('should normalize empty value patterns to empty string', () => {
    const patterns = ['null', 'NULL', 'Null', 'undefined', 'UNDEFINED', 'Undefined', 'N/A', 'n/a', 'N/a'];
...
describe('sanitizeMetadataValue', () => {
  it('should handle null and undefined values', () => {
    ...
    expect(nullResult.warnings[0].reason).toMatch(/Null\/undefined value converted to empty string/);

    const undefinedResult = sanitizeMetadataValue(undefined, 'test', 'text');
    expect(undefinedResult.value).toBe('');
    expect(undefinedResult.warnings[0].reason).toMatch(/Null\/undefined value converted to empty string/);
  });
