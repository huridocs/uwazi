...
describe('ItemList', () => {
  ...
  it('should pass the list search params as searchParams to the Doc', () => {
    ...
    render();
    expect(component.find(RowList).children().at(0).props().searchParams).toEqual({ sort: 'sortProperty' });
  });

  it('should default to sort: titel if no searchParams on link', () => {
    render();
    expect(component.find(RowList).children().at(0).props().searchParams).toEqual({ sort: 'title' });
  });
