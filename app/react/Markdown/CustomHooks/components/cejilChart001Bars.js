      ...
      return (
        <div style={{backgroundColor: '#fff', border: '1px solid #ccc'}}>
          ...
          <div style={{padding: '5px'}}>
            {this.props.payload[0].payload.casesLabel}:&nbsp;&nbsp;<b style={{color: '#600'}}>{this.props.payload[0].value}</b><br />
            {this.props.payload[1].payload.provisionalMeasuresLabel}:&nbsp;&nbsp;<b style={{color: '#600'}}>{this.props.payload[1].value}</b>
          </div>
        </div>
      );
    }
  ...
  mountData(props) {
    ...
    if (props.data) {
      fullData = props.data.map(item => {
        return {
          name: item.label,
          casesLabel: item.caseLabel,
          casesValue: item.caseResults,
          provisionalMeasuresLabel: item.provisionalMeasureLabel,
          provisionalMeasuresValue: item.provisionalMeasureResults,
          xAxisName: ''};
      });
    }
    ...
    return (
      <ResponsiveContainer height={320}>
        <BarChart height={300} data={this.state.fullData}
          ...
          <Bar dataKey="casesValue" fill="#D24040" shape={<ColoredBar />} stackId="unique" />
          <Bar dataKey="provisionalMeasuresValue" fill="#D24040" shape={<ColoredBar color="light" />} stackId="unique" />
          <Legend
