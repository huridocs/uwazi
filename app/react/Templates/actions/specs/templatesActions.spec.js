...
describe('templatesActions', () => {
  describe('async actions', () => {
    ...
    beforeEach(() => {
      ...
      .post(`${APIURL}templates/setasdefault`, { body: JSON.stringify({ testBackendResult: 'ok' }) })
      .get(`${APIURL}documents/count_by_template?templateId=templateWithDocuments`, { body: JSON.stringify(documentsUsingTemplate) })
      .get(`${APIURL}templates/count_by_thesauri?_id=templateWithDocuments`, { body: JSON.stringify(0) })
      .get(`${APIURL}documents/count_by_template?templateId=templateAsThesauri`, { body: JSON.stringify(0) })
      .get(`${APIURL}templates/count_by_thesauri?_id=templateAsThesauri`, { body: JSON.stringify(1) })
      .get(`${APIURL}documents/count_by_template?templateId=templateWithoutDocuments`, { body: JSON.stringify(0) })
      .get(`${APIURL}templates/count_by_thesauri?_id=templateWithoutDocuments`, { body: JSON.stringify(0) });
    ...
    describe('setAsDefault', () => {
      it('should set the template as the default template', async () => {
        ...
        expect(JSON.parse(backend.lastOptions(`${APIURL}templates/setasdefault`).body)).toEqual({ _id: 'default' });
      });
    });
  });
});
