  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="p-4 rounded-lg shadow-md">
        <h2 className="text-sm font-semibold font-roboto text-ink mb-3">
          ...
          {Object.entries(filters).map(([key, { count, status }]) => (
            <label key={key} className="flex items-center">
              <input
                ...
                className="h-4 w-4 rounded-sm border-border text-ink focus:ring-ink"
              />
              <span className="ml-3 flex flex-1 justify-between items-center text-sm text-ink-secondary">
                <span className="text-xs font-roboto">
                  <Translate>{statusKeys[key as EntityStatus]}</Translate>
                </span>
                <div className="border-b border-dashed border-border mx-2 grow" />
