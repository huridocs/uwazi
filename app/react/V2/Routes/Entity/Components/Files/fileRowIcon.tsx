...
const getExternalLinkIcon = (url: string) => {
  const platform = resolveExternalPlatform(url);
  if (platform === 'youtube') return <YouTubeIcon className={iconClass} />;
  if (platform === 'vimeo') return <VimeoIcon className={iconClass} />;
...
const getRowIcon = (row: EntityFileRow) => {
  ...
  if (row.fileType === 'externalURL' || url.startsWith('http')) {
    return getExternalLinkIcon(url);
  }
