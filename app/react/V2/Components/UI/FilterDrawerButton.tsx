...
const FilterDrawerButton = ({ activeCount, onClick, size = 'sm' }: FilterDrawerButtonProps) => {
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`relative inline-flex cursor-pointer items-center gap-1.5 rounded-md border ${
        height
      } ${padding} ${text} font-medium transition-colors ${
        active
    ...
    >
      ...
      {active && (
        <span
          ...
          className="inline-grid h-3.5 min-w-3.5 shrink-0 place-items-center rounded-full bg-ink px-1 text-[9px] font-semibold leading-0 text-parchment tabular-nums"
        >
          <span className="leading-none">{activeCount}</span>
        </span>
      )}
    </button>
  );
