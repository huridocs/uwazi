...
const GroupCell = <T extends TableRow<T>>({ row }: { row: Row<T> }) => {
  ...
  return canExpand ? (
    <EmbededButton
      ...
      color="indigo"
      className={`${expanded ? 'bg-parchment' : 'bg-warm'} rounded-md border-none drop-shadow-none`}
    >
      <Translate className={`${expanded ? 'text-ink' : 'text-ink-secondary'}`}>Group</Translate>
