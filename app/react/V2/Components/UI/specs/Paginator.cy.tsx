...
describe('Paginator', () => {
  ...
  it('should render with the current page and the correct basic links', () => {
    mount(
      <Basic currentPage={5} totalPages={25} buildUrl={(page: string) => `path?page=${page}`} />
    );
  ...
  it('should render a disabled button for the previous/next arrow if on first/last page', () => {
    mount(
      <Basic currentPage={1} totalPages={25} buildUrl={(page: string) => `path?page=${page}`} />
    );
    cy.get('a').eq(0).should('have.attr', 'href', '/path?page=1');
    cy.get('button').eq(0).should('have.attr', 'disabled');

    mount(
      <Basic currentPage={25} totalPages={25} buildUrl={(page: string) => `path?page=${page}`} />
    );
  ...
  describe('current page siblings', () => {
    it('should show not show them if they are the same as the first or last page', () => {
      mount(
        <Basic currentPage={2} totalPages={25} buildUrl={(page: string) => `path?page=${page}`} />
      );
      cy.get('a').eq(0).should('have.attr', 'href', '/path?page=1');
      cy.get('a').eq(1).should('have.attr', 'href', '/path?page=1');
      ...
      mount(
        <Basic currentPage={24} totalPages={25} buildUrl={(page: string) => `path?page=${page}`} />
      );
  ...
  describe('show more', () => {
    ...
    it('should not show more pages button if there are none', () => {
      mount(
        <Basic currentPage={2} totalPages={3} buildUrl={(page: string) => `path?page=${page}`} />
      );
    ...
    it('should not show more pages button if it is unnecessary', () => {
      mount(
        <Basic currentPage={2} totalPages={7} buildUrl={(page: string) => `path?page=${page}`} />
      );
      cy.contains('button', 'Show more').should('not.exist');

      mount(
        <Basic currentPage={1} totalPages={3} buildUrl={(page: string) => `path?page=${page}`} />
      );
      cy.contains('button', 'Show more').should('not.exist');

      mount(
        <Basic currentPage={5} totalPages={5} buildUrl={(page: string) => `path?page=${page}`} />
      );
      cy.contains('button', 'Show more').should('not.exist');
    });
  });
