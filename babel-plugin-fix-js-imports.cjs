    ...
    visitor: {
      StringLiteral(path) {
        ...
        if (
          ...
          (value.startsWith('./') || value.startsWith('../') || value.startsWith('#app/') || value.startsWith('#api/') || value.startsWith('#shared/') || value.startsWith('#V2/') || value.startsWith('#UI/')) &&
          ...
          const parent = path.findParent(p => 
            ...
            (t.isCallExpression(p.node) && t.isIdentifier(p.node.callee) && p.node.callee.name === 'require')
          );
