...
- [x] Mongo migration `205-backfill-translation-context` (`requiresSchema: 15`). Writes after 205 cannot omit `context.type` / `context.label`: locale saves inherit type/label or throw, mappers assert before persist, and 205 attaches a `translationsV2` JSON schema validator. Copy refuses to start until 205 is recorded.
...
- **Incomplete Mongo context:** `POST /api/translations` does not require `type`/`label`, and Mongo `$set`s the whole `context` object. Partial locale saves used to persist `{ id }` only. PG columns are `NOT NULL` (and `context_type` is CHECKed). Migration `205-backfill-translation-context` fills missing type/label, then attaches a `translationsV2` JSON schema validator so later Mongo writes cannot omit them before the Postgres cutover. Application writes inherit type/label from existing rows or throw; mappers assert a complete context before persist. Copy `assertPrerequisites` refuses to start unless Mongo migration 205 has been recorded. Unresolvable rows still fail loudly rather than inserting nulls.
