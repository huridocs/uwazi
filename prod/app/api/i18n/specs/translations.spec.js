...
describe('translations', () => {
  ...
  describe('process System context', () => {
    it('should add keys that do not exist into all languages', done => {
      ...
      _translations.default.processSystemKeys(keys).
      then(_translations.default.get).
      then(result => {
  ...
  describe('get()', () => {
    it('should return the translations', done => {
      ...
      then(result => {
        ...
        expect(result[0].contexts[3].id).toBe(_fixtures.entityTemplateId.toString());
        expect(result[0].contexts[3].type).toBe('Entity');

        expect(result[0].contexts[4].id).toBe(_fixtures.documentTemplateId.toString());
  ...
  describe('save()', () => {
    it('should save the translation and return it', done => {
      _translations.default.save({ locale: 'fr' }).
      then(result => {
    ...
    it('should transform values from map to array if its a map', done => {
      _translations.default.save({ locale: 'fr', contexts: [{ values: { test: 'value' } }, { values: [{ key: 'test2', value: 'value2' }] }] }).
      then(() => _translations.default.get()).
      then(result => {
    ...
    it('should save partial translations', done => {
      _translations.default.save({ _id: _fixtures.englishTranslation,
        locale: 'en',
        contexts: [{
          id: 'Filters',
          label: 'Filters',
      ...
      then(() => _translations.default.get({ _id: _fixtures.englishTranslation })).
      then(([translation]) => {
  ...
  describe('addEntry()', () => {
    it('should add the new key to each dictionary in the given context', done => {
      _translations.default.addEntry('System', 'Key', 'default').
      then(result => {
        expect(result).toBe('ok');
        return _translations.default.get();
      }).
      then(result => {
  ...
  describe('addContext()', () => {
    it('should add a context with his values', done => {
      ...
      _translations.default.addContext('context', 'Judge', values, 'type').
      then(result => {
        expect(result).toBe('ok');
        return _translations.default.get();
      }).
  ...
  describe('deleteContext()', () => {
    it('should add a context with his values', done => {
      _translations.default.deleteContext('System').
      then(result => {
        expect(result).toBe('ok');
        return _translations.default.get();
      }).
  ...
  describe('updateContext()', () => {
    it('should add values if the context values are undefined', done => {
      ...
      const context = { Pass: 'Pass', Acc: 'Acc', Email: 'Email', Name: 'Name', Interface: 'Interface' };

      _translations.default.updateContext('Menu', 'Menu', keyNameChanges, deletedProperties, context).
      then(result => {
        expect(result).toBe('ok');
        done();
      }).
    ...
    it('should add a context with his values', done => {
      const keyNameChanges = { Password: 'Pass', Account: 'Acc', System: 'Interface' };
      const deletedProperties = ['Age'];
      const values = { Pass: 'Pass', Acc: 'Acc', Email: 'Email', Name: 'Names', Interface: 'Interfaces' };

      _translations.default.updateContext('System', 'Interface', keyNameChanges, deletedProperties, values).
      then(result => {
        expect(result).toBe('ok');
        return _translations.default.get();
      }).
  ...
  describe('addLanguage', () => {
    it('should clone translations of default language and change language to the one added', async () => {
      await _translations.default.addLanguage('fr');
      const allTranslations = await _translations.default.get();
  ...
  describe('removeLanguage', () => {
    it('should remove translation for the language passed', async () => {
      await _translations.default.removeLanguage('es');
      await _translations.default.removeLanguage('other');
      const allTranslations = await _translations.default.get();
