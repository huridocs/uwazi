...
function updateEntity(entity, _template) {
  ...
  then(docLanguages => {
    ...
    return Promise.all(docs.map(d => _entitiesModel.default.save(d)));
  });
}
...
function createEntity(doc, languages, sharedId) {
  ...
  return _entitiesModel.default.save(docs);
}
...
function getEntityTemplate(doc, language) {
  return new Promise(resolve => {
    ...
    if (doc.template) {
      return _templates2.default.getById(doc.template).then(resolve);
    }
    ...
    then(storedDoc => {
      ...
      return _templates2.default.getById(storedDoc.template).then(resolve);
    });
  });
}
  ...
  const metadata = template.properties.reduce((sanitizedMetadata, { type, name }) => {
    ...
    if (type === 'daterange' && sanitizedMetadata[name]) {
      ...
      if (!value.to && !value.from) {
        const { [name]: dateRange } = sanitizedMetadata,withoutDateRange = _objectWithoutProperties(sanitizedMetadata, [name].map(_toPropertyKey));
...

{
  ...
  save(_doc, { user, language }, updateRelationships = true, index = true) {
    ...
    then(([entity]) => {
      if (updateRelationships) {
        return Promise.all([entity, _relationships.default.saveEntityBasedReferences(entity, language)]);
      }
  ...
  indexEntities(query, select, limit = 200, batchCallback = () => {}) {
    const index = (offset, totalRows) => {
      ...
      then(entities => Promise.all(entities.map(entity => _relationships.default.get({ entity: entity.sharedId }).
      then(relations => {
        entity.relationships = relations || [];
        return entity;
      })))).
      then(entities => _search.default.bulkIndex(entities).then(() => batchCallback(entities.length, totalRows))).
  ...
  async get(query, select, pagination) {
    const entities = await _entitiesModel.default.get(query, select, pagination);
    return entities;
  },

  async getWithRelationships(query, select, pagination) {
    const _entities = await _entitiesModel.default.get(query, select, pagination).
    then(entities => Promise.all(entities.map(entity => _relationships.default.getByDocument(entity.sharedId, entity.language).
    then(relations => {
      entity.relationships = relations;
      return entity;
    }))));
  ...
  countByTemplate(template) {
    return _entitiesModel.default.count({ template });
  },
  ...
  getByTemplate(template, language, onlyPublished = true) {
    ...
    return _entitiesModel.default.get(query, ['title', 'icon', 'file', 'sharedId']);
  },

  updateMetdataFromRelationships(entities, language) {
    const entitiesToReindex = [];
    return _templates2.default.get().
    then(_templates => Promise.all(
    entities.map(entityId => Promise.all([this.getById(entityId, language), _relationships.default.getByDocument(entityId, language)]).
  ...
  updateMetadataProperties(template, currentTemplate, language) {
    ...
    template.properties = (0, _utils.generateNamesAndIds)(template.properties); //eslint-disable-line
    ...
    if (actions.$unset || actions.$rename) {
      dbUpdate = _entitiesModel.default.db.updateMany({ template }, actions);
    }
  ...
  deleteFiles(deletedDocs) {
    ...
    reduce((filePaths, doc) => {
      if (doc.file) {
        filePaths.push(_path.default.normalize(`${_paths.default.uploadedDocuments}/${doc.file.filename}`));
        filePaths.push(_path.default.normalize(`${_paths.default.uploadedDocuments}/${doc._id.toString()}.jpg`));
      }

      if (doc.attachments) {
        doc.attachments.forEach(file => filePaths.push(_path.default.normalize(`${_paths.default.uploadedDocuments}/${file.filename}`)));
      }

      return filePaths;
    }, []);
    filesToDelete = filesToDelete.filter((doc, index) => filesToDelete.indexOf(doc) === index);
    return (0, _files.deleteFiles)(filesToDelete).
    catch(error => {
  ...
  deleteIndexes(sharedIds) {
    const deleteIndexBatch = (offset, totalRows) => {
      ...
      then(entities => _search.default.bulkDelete(entities)).
  ...
  delete(sharedId, deleteIndex = true) {
    ...
    then(docs => deleteIndex ? Promise.all(docs.map(doc => _search.default.delete(doc))).then(() => docs) : Promise.resolve(docs)).
    then(docs => _entitiesModel.default.delete({ sharedId }).
    then(() => docs).
  ...
  async getRawPage(sharedId, language, pageNumber) {
    const [entity] = await _entitiesModel.default.get({ sharedId, language }, { [`fullText.${pageNumber}`]: true });
    if (!entity || !entity.fullText) {
      throw (0, _Error.default)('entity does not exists', 404);
    }

    if (typeof entity.fullText[pageNumber] === 'undefined') {
      throw (0, _Error.default)('page does not exist', 404);
    }
  ...
  removeValuesFromEntities(properties, template) {
    ...
    _entitiesModel.default.db.updateMany(query, { $set: changes })]).
  ...
  deleteEntityFromMetadata(sharedId, propertyContent) {
    return _templates2.default.get({ 'properties.content': propertyContent }).
    then(allTemplates => {
      ...
      selectQuery.$or.length ? _entitiesModel.default.db.updateMany(selectQuery, { $set: selectChanges }) : null,
      multiSelectQuery.$or.length ? _entitiesModel.default.db.updateMany(multiSelectQuery, { $pull: multiSelectChanges }) : null]).
  ...
  async createThumbnail(entity) {
    const filePath = _path.default.join(_paths.default.uploadedDocuments, entity.file.filename);
    return new _PDF.default({ filename: filePath }).createThumbnail(entity._id.toString());
  },

  async deleteLanguageFiles(entity) {
    const filesToDelete = [];
    filesToDelete.push(_path.default.normalize(`${_paths.default.uploadedDocuments}/${entity._id.toString()}.jpg`));
    ...
    if (entity.file) {
      ...
      if (shouldUnlinkFile) {
        filesToDelete.push(_path.default.normalize(`${_paths.default.uploadedDocuments}/${entity.file.filename}`));
      }
    }
    if (entity.file) {return (0, _files.deleteFiles)(filesToDelete);}
  },
  ...
  async removeLanguage(locale) {
    ...
    then(() => _search.default.deleteLanguage(locale));
  },

  count: _entitiesModel.default.count };exports.default = _default;