...
const sanitizeConfig = async config => Object.keys(config).reduce(async (prev, key) => {
  ...
  if (key === 'templates') {
    const templatesData = await _odm.models.templates.get({});
...
const getValuesFromTemplateProperties = async (config, validTypes, valueProperty) => {
  ...
  return Object.keys(templatesConfig).reduce(async (prev, templateId) => {
    ...
    const template = await _odm.models.templates.getById(templateId);
...
const oneSecond = 1000;var _default =
...
async config => {
  ...
  return {
    ...
    async shouldSync(change) {
      ...
      let data = await _odm.models[change.namespace].getById(change.mongoId);
      ...
      if (change.namespace === 'connections') {
        const entitiesData = await _odm.models.entities.get({ sharedId: data.entity });
        ...
        const templateData = await _odm.models.templates.getById(entityTemplate);
        ...
        const hubOtherConnections = await _odm.models.connections.get({ hub: data.hub, _id: { $ne: data._id } });
        const hubOtherEntities = await _odm.models.entities.get({ sharedId: { $in: hubOtherConnections.map(h => h.entity) } });
        ...
        const hubOtherTemplates = await _odm.models.templates.get({ _id: { $in: hubWhitelistedTemplateIds } });
      ...
      if (change.namespace === 'translations') {
        const templatesData = await _odm.models.templates.get({ _id: { $in: Object.keys(templatesConfig) } });
      ...
      if (change.namespace === 'entities' && data.metadata) {
        const templateData = await _odm.models.templates.getById(data.template);
