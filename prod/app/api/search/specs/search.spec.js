...
var _elasticResult = _interopRequireDefault(require("./elasticResult"));
var _fixtures_elastic = _interopRequireWildcard(require("./fixtures_elastic"));function _interopRequireWildcard(obj) {if (obj && obj.__esModule) {return obj;} else {var newObj = {};if (obj != null) {for (var key in obj) {if (Object.prototype.hasOwnProperty.call(obj, key)) {var desc = Object.defineProperty && Object.getOwnPropertyDescriptor ? Object.getOwnPropertyDescriptor(obj, key) : {};if (desc.get || desc.set) {Object.defineProperty(newObj, key, desc);} else {newObj[key] = obj[key];}}}}newObj.default = obj;return newObj;}}function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} /* eslint-disable max-nested-callbacks */
...
describe('search', () => {
  ...
  beforeAll(async () => {
    result = (0, _elasticResult.default)().withDocs(
    [
    { title: 'doc1',
      _id: 'id1',
      snippets: {
  ...
  describe('getUploadsByUser', () => {
    it('should request all unpublished entities or documents for the user', async () => {
      const user = { _id: _fixtures_elastic.ids.userId };
      const response = await _.search.getUploadsByUser(user, 'en');
  ...
  describe('search', () => {
    describe('searchSnippets', () => {
      it('perform a search on fullText of the document passed and return the snippets', done => {
        _.search.searchSnippets('spanish', _fixtures_elastic.ids.batmanFinishes, 'es').
        then(snippets => {
      ...
      it('perform a search on metadata and fullText and return the snippets', done => {
        _.search.searchSnippets('gargoyles', _fixtures_elastic.ids.metadataSnippets, 'en').
        then(snippets => {
      ...
      it('should perform the search on unpublished documents also', done => {
        _.search.searchSnippets('unpublished', 'unpublishedSharedId', 'en').
        then(snippets => {
          expect(snippets.fullText.length).toBe(1);
          done();
        }).
      ...
      describe('when document is not matched', () => {
        it('should return snippet object with 0 count and empty arrays', done => {
          _.search.searchSnippets('not matching string', _fixtures_elastic.ids.batmanFinishes, 'es').
          then(snippets => {
      ...
      describe('when searchTerm is empty', () => {
        it('should return empty array', done => {
          _.search.searchSnippets('', _fixtures_elastic.ids.batmanFinishes, 'es').
          then(snippets => {
    ...
    it('should perform a fullTextSearch on passed fields', done => {
      Promise.all([
      _.search.search({ searchTerm: 'spanish', fields: ['title'] }, 'es'),
      _.search.search({ searchTerm: 'Batman', fields: ['title'] }, 'es')]).
    ...
    it('should perform a fullTextSearch on fullText and title', done => {
      Promise.all([
      _.search.search({ searchTerm: 'spanish' }, 'es'),
      _.search.search({ searchTerm: 'english' }, 'es'),
      _.search.search({ searchTerm: 'english' }, 'en'),
      _.search.search({ searchTerm: 'finishes' }, 'en'),
      _.search.search({ searchTerm: 'Batman begins NOT finishes' }, 'es'),
      _.search.search({ searchTerm: 'begins OR finishes' }, 'es'),
      _.search.search({ searchTerm: 'Batman' }, 'en'),
      _.search.search({ searchTerm: 'document english' }, 'en'),
      _.search.search({ searchTerm: '"document english"' }, 'en')]).
    ...
    it('should return aggregations when searching by 2 terms', done => {
      _.search.search({ searchTerm: 'english document' }, 'es').
      then(response => {
        const aggregation = response.aggregations.all._types.buckets.find(bucket => bucket.key === _fixtures_elastic.ids.template1.toString());
    ...
    it('should convert aggregation buckets that are objects to arrays', done => {
      ...
      spyOn(_.elastic, 'search').and.returnValue(Promise.resolve(result));
      _.search.search({ searchTerm: '', geolocation: true }, 'en').
      then(response => {
    ...
    it('should match entities related somehow with other entities with a title that is the search term', done => {
      _.search.search({ searchTerm: 'egypt' }, 'en').
      then(({ rows }) => {
    ...
    it('should limit the results', done => {
      _.search.search({ searchTerm: '', limit: 1, sort: 'title' }, 'en').
      then(({ rows }) => {
    ...
    it('should return results from a given number', done => {
      _.search.search({ searchTerm: '', limit: 1, sort: 'title', from: 1 }, 'en').
      then(({ rows }) => {
        expect(rows.length).toBe(1);
    ...
    it('should filter by templates', done => {
      Promise.all([
      _.search.search({ types: [_fixtures_elastic.ids.template1] }, 'es'),
      _.search.search({ types: [_fixtures_elastic.ids.template2] }, 'es'),
      _.search.search({ types: [_fixtures_elastic.ids.template1] }, 'en'),
      _.search.search({ types: [_fixtures_elastic.ids.template1, _fixtures_elastic.ids.template2] }, 'en'),
      _.search.search({ types: ['missing'] }, 'en'),
      _.search.search({ types: [_fixtures_elastic.ids.template1, 'missing'] }, 'en')]).
    ...
    it('should allow searching only within specific Ids', done => {
      Promise.all([
      _.search.search({ ids: [_fixtures_elastic.ids.batmanBegins] }, 'es'),
      _.search.search({ ids: _fixtures_elastic.ids.batmanBegins }, 'en'),
      _.search.search({ ids: [_fixtures_elastic.ids.batmanFinishes, _fixtures_elastic.ids.batmanBegins] }, 'en')]).
    ...
    it('should filter by metadata, and return template aggregations based on the filter the language and the published status', done => {
      Promise.all([
      _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2], filters: { field1: 'joker' } }, 'en'),
      _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2], unpublished: true }, 'en', { _id: 'user' })]).

      then(([joker, unpublished]) => {
        expect(joker.rows.length).toBe(2);

        const typesAggs = joker.aggregations.all._types.buckets;
        expect(typesAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata1).filtered.doc_count).toBe(2);
        expect(typesAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata2).filtered.doc_count).toBe(0);

        const unpublishedAggs = unpublished.aggregations.all._types.buckets;
        expect(unpublishedAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata1).filtered.doc_count).toBe(1);
        expect(unpublishedAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata2).filtered.doc_count).toBe(0);
        done();
      }).
      catch((0, _jasmineHelpers.catchErrors)(done));
    });
    ...
    it('should filter by relationships metadata selects', async () => {
      const response = await _.search.search({
    ...
    it('should filter by relationships metadata markdown', async () => {
      const response = await _.search.search({
        types: [_fixtures_elastic.ids.templateMetadata1],
    ...
    it('should filter by fullText, and return template aggregations based on the filter the language and the published status', done => {
      ...
      then(([matches]) => {
        ...
        expect(matchesAggs.find(a => a.key === _fixtures_elastic.ids.template1).filtered.doc_count).toBe(1);
        expect(matchesAggs.find(a => a.key === _fixtures_elastic.ids.template2).filtered.doc_count).toBe(0);
        expect(matchesAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata1).filtered.doc_count).toBe(0);
        expect(matchesAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata2).filtered.doc_count).toBe(0);
        done();
      }).
      catch((0, _jasmineHelpers.catchErrors)(done));
    });
    ...
    describe('when the query is for geolocation', () => {
      it('should set size to 9999', done => {
        ...
        _.search.search({ searchTerm: '', geolocation: true }, 'en').
        then(() => {
          const elasticQuery = _.elastic.search.calls.argsFor(0)[0].body;
    ...
    describe('select aggregations', () => {
      it('should return aggregations of select fields when filtering by types', done => {
        Promise.all([
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1] }, 'en'),
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata2] }, 'en'),
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2] }, 'en'),
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1], unpublished: true }, 'en', { _id: 'user' })]).
    ...
    describe('multiselect aggregations', () => {
      it('should return aggregations of multiselect fields', done => {
        ...
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata2] }, 'en'),
        _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2] }, 'en'),
        _.search.search({ filters: { multiselect1: { values: ['multiValue2'], and: false } },
          types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2] }, 'en')]).
        ...
        then(([template1, template2, both, filtered]) => {
          ...
          expect(templateAggs.find(a => a.key === _fixtures_elastic.ids.template1).filtered.doc_count).toBe(0);
          expect(templateAggs.find(a => a.key === _fixtures_elastic.ids.template2).filtered.doc_count).toBe(0);
      ...
      describe('allAggregations', () => {
        it('should return all aggregations', async () => {
          const allAggregations = await _.search.search({ allAggregations: true }, 'en');
      ...
      describe('AND flag', () => {
        it('should not fail when no values sent', async () => {
          const filtered = await _.search.search({
            filters: { multiselect1: { and: true } },
            types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2] },
          'en');
        ...
        it('should restrict the results to those who have all values of the filter', done => {
          ...
          then(filtered => {
            ...
            expect(templateAggs.find(a => a.key === _fixtures_elastic.ids.templateMetadata1).filtered.doc_count).toBe(1);
      ...
      describe('nested', () => {
        it('should search by nested and calculate nested aggregations of fields when filtering by types', done => {
          Promise.all([
          _.search.search({ types: [_fixtures_elastic.ids.templateMetadata2] }, 'en'),
          _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2],
    ...
    it('should sort (ignoring case and leading whitespaces) if sort is present', done => {
      Promise.all([
      _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2], order: 'asc', sort: 'title' }, 'en'),
      _.search.search({ types: [_fixtures_elastic.ids.templateMetadata1, _fixtures_elastic.ids.templateMetadata2], order: 'desc', sort: 'title' }, 'en')]).
    ...
    it('should allow including unpublished documents if user', done => {
      spyOn(_.elastic, 'search').and.returnValue(new Promise(resolve => resolve(result)));
      _.search.search({
        searchTerm: 'searchTerm',
        includeUnpublished: true },
      'es', 'user').
      then(() => {
        const expectedQuery = (0, _.documentQueryBuilder)().
        fullTextSearch('searchTerm', ['metadata.field1', 'metadata.field2', 'metadata.rich_text', 'metadata.field3', 'title', 'fullText'], 2).
        includeUnpublished().
        language('es').
        query();

        expect(_.elastic.search).toHaveBeenCalledWith({ index: _elasticIndexes.default.index, body: expectedQuery });
        done();
      });
    });
