"use strict";var _reduxThunk = _interopRequireDefault(require("redux-thunk"));
...
describe('Relationships actions', () => {
  ...
  describe('updateRightRelationshipType', () => {
    it('should UPDATE_RELATIONSHIPS_RIGHT_TYPE passing newRightRelationshipType as false if right index not last', () => {
      ...
      const getState = () => ({ relationships: { hubs: (0, _immutable.fromJS)([0, 1, 2, { rightRelationships: [0, 1, 2, 3, 4, 5, 6, 7] }]) } });
  ...
  describe('reloadRelationships', () => {
    ...
    it('should call on routeUtils and set the LIST results', done => {
      ...
      then(() => {
        ...
        new _RequestParams.RequestParams({ sharedId: 'parentEntityId' }),
        { templates });
  ...
  describe('saveRelationships', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(_api.default, 'post').and.returnValue(Promise.resolve('POSTresponse'));
      spyOn(_api.default, 'get').and.returnValue(Promise.resolve({ json: { rows: ['entity'] } }));
    ...
    it('should post to relationship/bluk with empty actions if no changes made', done => {
      actions.saveRelationships()(store.dispatch, getState).
      then(() => {
        expect(_api.default.post).toHaveBeenCalledWith('relationships/bulk', new _RequestParams.RequestParams({ save: [], delete: [] }));
        done();
      });
    });
    ...
    describe('when changes have taken place', () => {
      beforeEach(() => {
        ...
        push((0, _immutable.fromJS)({ entity: 'n7', template: '1' })));
        ...
        push((0, _immutable.fromJS)({ entity: 'n8', template: '1' })));
        ...
        push((0, _immutable.fromJS)({ entity: 'n9', template: '1', deleted: true })));
        hubs = hubs.setIn([2, 'rightRelationships', 0, 'relationships'],
        hubs.
        getIn([2, 'rightRelationships', 0, 'relationships']).
        push((0, _immutable.fromJS)({ entity: 'n10', template: '1' })));
      ...
      it('should call on bulk', done => {
        actions.saveRelationships()(store.dispatch, getState).
        then(() => {
          expect(_api.default.post.calls.mostRecent().args[0]).toBe('relationships/bulk');
          done();
        });
      });
      ...
      it('should handle new hubs, taking into account deleted sections on new hubs', done => {
        actions.saveRelationships()(store.dispatch, getState).
        then(() => {
          expect(_api.default.post.calls.mostRecent().args[1].data.save).toContainEqual([
      ...
      it('should handle modifications, taking into account new relationships on existing hubs and deleted relationships / sections', done => {
        actions.saveRelationships()(store.dispatch, getState).
        then(() => {
          expect(_api.default.post.calls.mostRecent().args[1].data.save).
          ...
          expect(_api.default.post.calls.mostRecent().args[1].data.save).toContainEqual({ entity: 'n7', hub: 'hub1', template: '1' });
          expect(_api.default.post.calls.mostRecent().args[1].data.save).
          toContainEqual({ entity: 'entityId', hub: 'hub2', _id: 'originalLeftRelationship2', template: '1' });
          expect(_api.default.post.calls.mostRecent().args[1].data.save).toContainEqual({ entity: 'n10', hub: 'hub3', template: '1' });
      ...
      it('should handle deletions, taking into account new hubs and relationships', done => {
        actions.saveRelationships()(store.dispatch, getState).
        then(() => {
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalRightRelationship3', entity: 'o3' });
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalRightRelationship5', entity: 'o5' });
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalRightRelationship7', entity: 'o7' });
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalRightRelationship8', entity: 'o8' });
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalRightRelationship9', entity: 'o9' });
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toContainEqual({ _id: 'originalLeftRelationship3', entity: 'entityId' });
      ...
      it('should handle all actions simultaneously', done => {
        ...
        then(() => {
          ...
          expect(_api.default.post.calls.mostRecent().args[1].data.save).toEqual(expectedSaves);
          expect(_api.default.post.calls.mostRecent().args[1].data.delete).toEqual(expectedDeletes);
          done();
        });
      });
    });
  ...
  describe('Search-related actions', () => {
    ...
    beforeEach(() => {
      store = mockStore({});
      spyOn(_api.default, 'get').and.returnValue(Promise.resolve({ json: { rows: [{ type: 'entity' }, { type: 'doc' }] } }));
    });

    describe('immidiateSearch', () => {
      it('should search for connectable entities', () => {
        actions.immidiateSearch(store.dispatch, 'term');
        expect(_api.default.get).toHaveBeenCalledWith('search', new _RequestParams.RequestParams({ searchTerm: 'term', fields: ['title'], includeUnpublished: true }));
    ...
    describe('search', () => {
      it('should update the state searchTerm and debounce server searching the term', () => {
        ...
        expect(_api.default.get).not.toHaveBeenCalled();
