...
const conformRelationships = action => {
  ...
  return hubsObject.reduce((hubs, hub) => {
    ...
    const rightRelationships = hub.get('rightRelationships').reduce((memo, relationshipsArray, template) => {
      let newMemo = memo.push((0, _immutable.fromJS)({}).set('template', template).set('relationships', relationshipsArray));
      index += 1;
  ...
  switch (action.type) {
    ...
    case types.UPDATE_RELATIONSHIPS_RIGHT_TYPE:
      ...
      if (action.newRightRelationshipType) {
        const updatedRightRelationships = updatedHubs.getIn([action.index, 'rightRelationships']).push((0, _immutable.fromJS)(emptyRigthRelationship()));
    ...
    case types.ADD_RELATIONSHIPS_ENTITY:
      ...
      push((0, _immutable.fromJS)({ template: relationship.get('template'), entity: action.entity.sharedId, entityData: action.entity }));
