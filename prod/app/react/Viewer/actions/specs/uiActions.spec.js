...
var actions = _interopRequireWildcard(require("../uiActions"));
...
describe('Viewer uiActions', () => {
  ...
  describe('activateReference()', () => {
    ...
    it('should scroll to the elements', done => {
      actions.activateReference({ _id: 'id' }, {})(dispatch);
      setTimeout(() => {
        expect(_Scroller.default.to).toHaveBeenCalledWith('.document-viewer a[data-id="id"]', '.document-viewer', { duration: 50 });
        expect(_Scroller.default.to).toHaveBeenCalledWith('.metadata-sidepanel .item-id', '.metadata-sidepanel .sidepanel-body', { duration: 50 });
        done();
      });
    });
  });
  ...
  describe('highlightSnippet', () => {
    it('should unmark all and mark snippets passed only once (only the ones for the pages being rendered)', () => {
      ...
      actions.highlightSnippet(_immutable.default.fromJS({ text: 'snippet <b>marked</b> (with) multiple spaces', page: 3 }));
      ...
      actions.highlightSnippet(_immutable.default.fromJS({}));
