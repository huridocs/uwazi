"use strict";var _batarange = _interopRequireDefault(require("batarange"));

var _Text = _interopRequireDefault(require("../Text"));
var _mockRangeSelection = _interopRequireDefault(require("../../../utils/mockRangeSelection"));
var _wrapper = _interopRequireDefault(require("../../../utils/wrapper"));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
describe('Text', () => {
  ...
  describe('getSelection', () => {
    it('should return a serialized selection', () => {
      spyOn(_batarange.default, 'serialize').and.returnValue({ start: 1, end: 10 });
      const range = (0, _mockRangeSelection.default)('text');

      const serializedRange = text.getSelection();
      expect(serializedRange).toEqual({ start: 1, end: 10, text: 'text' });
      expect(_batarange.default.serialize).toHaveBeenCalledWith(range, document);
    });
  ...
  describe('simulateSelection', () => {
    describe('when there is no selection', () => {
      it('should wrap the range with a span.fake-selection', () => {
        spyOn(_wrapper.default, 'wrap').and.returnValue('fakeSelection');
        spyOn(_batarange.default, 'restore').and.returnValue('restoredRange');
        ...
        expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 1, end: 2 }, document);
        expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper, 'restoredRange');
      ...
      describe('when the range rendered does not match the range selected', () => {
        it('should not render the selection', () => {
          ...
          expect(_batarange.default.restore).not.toHaveBeenCalled();
        });
      });
    });
    ...
    describe('when selection passed is null', () => {
      it('should only remove current fakeSelection', () => {
        spyOn(text, 'removeSimulatedSelection');
        spyOn(_wrapper.default, 'wrap');
        text.simulateSelection(null);

        expect(text.removeSimulatedSelection).toHaveBeenCalled();
        expect(_wrapper.default.wrap).not.toHaveBeenCalled();
      });
    });
  ...
  describe('removeSimulatedSelection', () => {
    ...
    describe('when selection range ends just before the current char range', () => {
      it('should not wrap fake selection', () => {
        spyOn(_wrapper.default, 'wrap');
        spyOn(_batarange.default, 'restore');
        spyOn(text, 'removeSimulatedSelection');
        spyOn(text, 'selected').and.returnValue(false);
  ...
  describe('renderReferences', () => {
    ...
    beforeEach(() => {
      ...
      spyOn(_wrapper.default, 'wrap').and.returnValue({ unwrap });
      spyOn(_batarange.default, 'restore').and.returnValue('restoredRange');
    });
    ...
    it('should wrap a collection of references using range by default', () => {
      ...
      text.renderReferences(references);
      expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 1, end: 2 }, document);
      expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 3, end: 4 }, document);
      expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('1'), 'restoredRange');
      expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('2'), 'restoredRange');
    });
    ...
    it('should wrap a collection of references using identifier passed', () => {
      ...
      expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 1, end: 2 }, document);
      expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 3, end: 4 }, document);
      expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('1', 'identifier'), 'restoredRange');
      expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('2', 'identifier'), 'restoredRange');
    });
    ...
    it('should not render references already rendered', () => {
      ...
      _batarange.default.restore.calls.reset();
      _wrapper.default.wrap.calls.reset();
      text.renderReferences(secondReferences);

      expect(_batarange.default.restore.calls.count()).toBe(1);
      expect(_wrapper.default.wrap.calls.count()).toBe(1);
      expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 5, end: 6 }, document);
      expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('3'), 'restoredRange');
    });
    ...
    describe('with range', () => {
      it('should only render references inside the range adding the start offset of the range being rendered', () => {
        ...
        text.renderReferences(references);
        expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 4, end: 35 }, document);
        expect(_batarange.default.restore).toHaveBeenCalledWith({ start: 0, end: 6 }, document);
        expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('1'), 'restoredRange');
        expect(_wrapper.default.wrap).toHaveBeenCalledWith(elementWrapper('2'), 'restoredRange');
      });
    });
