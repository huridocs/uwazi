...
describe('Map', () => {
  ...
  const render = () => {
    component = (0, _enzyme.shallow)(_react.default.createElement(_Map.default, props), { disableLifecycleMethods: true });
    ...
    markers = component.find(_reactMapGl.Marker);
  };
  ...
  describe('render', () => {
    ...
    it('should render a ReactMapGL with the props', () => {
      const reactMap = component.find(_reactMapGl.default);
    ...
    it('should render a Markers component', () => {
      ...
      expect(firstMarker().find(_UI.Icon).props().icon).toBe('map-marker');
    ...
    it('should use custom renderMarker method', () => {
      props.renderMarker = (marker, onClick) => _jsx("div", { className: "custom-class", onClick: onClick });
      render();
  ...
  describe('updateDataSource', () => {
    it('should update the clusters data layer of the map style', () => {
      props.cluster = true;
      render();
      const { mapStyle } = component.find(_reactMapGl.default).props();
  ...
  describe('zoom behavior', () => {
    it('should zoom in and out', () => {
      ...
      Object.keys(_helper.TRANSITION_PROPS).forEach(prop => {
        if (prop !== 'transitionDuration') {
          expect(instance._onViewStateChange.calls.mostRecent().args[0][prop]).toBe(_helper.TRANSITION_PROPS[prop]);
        } else {
  ...
  describe('clicking on a marker', () => {
    beforeEach(() => {
      render();
      firstMarker().find(_UI.Icon).first().simulate('click');
      component.update();
    });
  ...
  describe('hovering a marker', () => {
    ...
    beforeEach(() => {
      render();
      firstMarker().find(_UI.Icon).first().simulate('mouseOver');
      component.update();
      popup = component.find(_reactMapGl.Popup);
    });
    ...
    it('should not render a popup when has no info', () => {
      secondMarker().find(_UI.Icon).first().simulate('mouseOver');
      component.update();
      popup = component.find(_reactMapGl.Popup);
