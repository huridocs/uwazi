...
var _PDFPage = _interopRequireDefault(require("../PDFPage.js"));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };} // import { PDFJS } from '../../../../../node_modules/pdfjs-dist/web/pdf_viewer.js';
...
describe('PDF', () => {
  ...
  beforeEach(() => {
    spyOn(_PDFJS.default, 'getDocument').and.returnValue({ promise: Promise.resolve(pdfObject) });
    props = {
      file: 'file_url',
      filename: 'original.pdf',
      onLoad: jasmine.createSpy('onLoad'),
      pdfInfo: _immutable.default.fromJS({
        1: { chars: 10 },
        2: { chars: 20 },
        3: { chars: 30 },
        4: { chars: 40 },
  ...
  const render = () => {
    component = (0, _enzyme.shallow)(_react.default.createElement(_PDF.default, props));
  ...
  describe('on instance', () => {
    it('should get the pdf with pdfjs', done => {
      render();
      expect(_PDFJS.default.getDocument).toHaveBeenCalledWith(props.file);
      setTimeout(() => {
  ...
  describe('on componentWillReceiveProps', () => {
    it('should not attempt to get the PDF if filname remains unchanged', () => {
      ...
      expect(_PDFJS.default.getDocument.calls.count()).toBe(1);
    });
    ...
    it('should get the new PDF if filename changed', done => {
      ...
      expect(_PDFJS.default.getDocument.calls.count()).toBe(2);
      setTimeout(() => {
  ...
  describe('render', () => {
    it('should render a pdfPage for each page', () => {
      ...
      component.update();
      expect(component.find(_PDFPage.default).length).toBe(3);
    });
  });
