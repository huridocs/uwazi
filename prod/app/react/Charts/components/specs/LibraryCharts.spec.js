...
var _LibraryCharts = require("../LibraryCharts");
var _LibraryChart = _interopRequireDefault(require("../LibraryChart"));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
describe('LibraryCharts', () => {
  ...
  const render = () => {
    component = (0, _enzyme.shallow)(_react.default.createElement(_LibraryCharts.LibraryCharts, props));
  };
  ...
  beforeEach(() => {
    ...
    state = {
      locale: 'es',
      translations: (0, _immutable.fromJS)([
      {
        locale: 'es',
        contexts: [
        {
  ...
  describe('When no fields found', () => {
    beforeEach(() => {
      props = {
        aggregations: (0, _immutable.fromJS)({
          all: { types: { buckets: [
        ...
        fields: (0, _immutable.fromJS)([]),
        collection: (0, _immutable.fromJS)({ filters: [] }),
    ...
    it('should render templates types on LibraryChart', () => {
      render();
      expect(component.find(_LibraryChart.default).length).toBe(1);
      const LibraryChartElement = component.find(_LibraryChart.default);
    ...
    it('should add the results of sub-items in filters', () => {
      props.collection = (0, _immutable.fromJS)({ filters: [
  ...
  describe('When no valid fields found', () => {
    beforeEach(() => {
      props = {
        ...
        fields: (0, _immutable.fromJS)([{ type: 'not-valid' }]),
        collection: (0, _immutable.fromJS)({ filters: [] }),
        templates: (0, _immutable.fromJS)([
        { _id: 't1', name: 't1name' },
        { _id: 't2', name: 't2name' },
  ...
  describe('When fields found', () => {
    ...
    it('should render one LibraryChart for each supported field with sorted options', () => {
      ...
      const LibraryChartElement1 = component.find(_LibraryChart.default).at(0);
  ...
  describe('mapStateToProps', () => {
    beforeEach(() => {
      state = {
        a: { aggregations: 'a', filters: (0, _immutable.fromJS)({ properties: 'a', documentTypes: ['dt1'] }) },
        b: { aggregations: 'b', filters: (0, _immutable.fromJS)({ properties: 'b', documentTypes: ['dt2'] }) },
    ...
    it('should return aggregations, fields and translation context according to store key', () => {
      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'a' }).aggregations).toBe('a');
      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'b' }).aggregations).toBe('b');
      expect((0, _LibraryCharts.mapStateToProps)(state, {}).aggregations).toBe(null);

      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'a' }).fields).toBe('a');
      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'b' }).fields).toBe('b');
      expect((0, _LibraryCharts.mapStateToProps)(state, {}).fields).toBe(null);

      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'a' }).translationContext).toBe('dt1');
      expect((0, _LibraryCharts.mapStateToProps)(state, { storeKey: 'b' }).translationContext).toBe('dt2');
      expect((0, _LibraryCharts.mapStateToProps)(state, {}).translationContext).toBe(null);
    });

    it('should return collection and templates', () => {
      expect((0, _LibraryCharts.mapStateToProps)(state, {}).collection).toBe('collection');
      expect((0, _LibraryCharts.mapStateToProps)(state, {}).templates).toBe('templates');
    });
  });
});