"use strict";var _react = _interopRequireDefault(require("react"));
var _enzyme = require("enzyme");
var _immutable = require("immutable");
var _reactReduxForm = require("react-redux-form");

var _FiltersForm = require("../FiltersForm");
var _FiltersFromProperties = _interopRequireDefault(require("../FiltersFromProperties"));function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}


describe('FiltersForm', () => {
  let component;
  let props;

  beforeEach(() => {
    props = {
      searchDocuments: jasmine.createSpy('searchDocuments'),
      fields: (0, _immutable.fromJS)([
      { _id: '1', name: 'name' },
      { _id: '2', name: 'name', type: 'numeric' },
      { _id: '3', name: 'date', type: 'date' },
      { _id: '4', name: 'nested', type: 'nested' },
      {
        _id: '5',
        name: 'select',
        type: 'select',
        content: 'thesauri1',
        options: [
        { id: 'a', label: 'a', value: 'a', results: 1 },
        { id: 'b', label: 'b', value: 'b', results: 1 },
        { id: 'c', label: 'c', value: 'c', results: 1, values: [{ id: 'd', label: 'd', value: 'd', results: 1 }] }] },


      {
        _id: '6',
        name: 'multiselect',
        type: 'multiselect',
        content: 'thesauri1',
        options: [
        { id: 'a', label: 'a', value: 'a', results: 1 },
        { id: 'b', label: 'b', value: 'b', results: 1 },
        { id: 'c', label: 'c', value: 'c', values: [{ id: 'd', label: 'd', value: 'd', results: 1 }], results: 1 }] }]),



      documentTypes: (0, _immutable.fromJS)([]),
      templates: (0, _immutable.fromJS)([]),
      aggregations: (0, _immutable.fromJS)({
        all: {
          select: {
            buckets: [
            { key: 'a', filtered: { doc_count: 1 } },
            { key: 'b', filtered: { doc_count: 1 } },
            { key: 'c', filtered: { doc_count: 1 } },
            { key: 'd', filtered: { doc_count: 1 } }] },


          multiselect: {
            buckets: [
            { key: 'a', filtered: { doc_count: 1 } },
            { key: 'b', filtered: { doc_count: 1 } },
            { key: 'c', filtered: { doc_count: 1 } },
            { key: 'd', filtered: { doc_count: 1 } }] } } }),




      search: { searchTerm: 'Batman' },
      storeKey: 'library' };

    component = (0, _enzyme.shallow)(_react.default.createElement(_FiltersForm.FiltersForm, props));
  });

  describe('form on submit', () => {
    it('should call searchDocuments, with the searchTerm', () => {
      component.find(_reactReduxForm.Form).simulate('submit', { myfilter: true });
      expect(props.searchDocuments).toHaveBeenCalledWith({ search: { myfilter: true } }, 'library');
    });
  });

  describe('render()', () => {
    it('should render diferent type fileds', () => {
      const fields = component.find(_FiltersFromProperties.default);
      expect(fields).toMatchSnapshot();
    });
  });

  describe('maped state', () => {
    it('should contain the fields', () => {
      const store = {
        library: {
          ui: (0, _immutable.fromJS)({ searchTerm: 'do a barrel roll' }),
          filters: (0, _immutable.fromJS)({ properties: [{ name: 'author' }], documentTypes: { a: true } }) },

        form: {
          filters: 'filtersForm' },

        templates: (0, _immutable.fromJS)([]),
        settings: { collection: (0, _immutable.fromJS)({}) } };

      const state = (0, _FiltersForm.mapStateToProps)(store, { storeKey: 'library' });
      expect(state.fields.toJS()).toEqual([{ name: 'author' }]);
      expect(state.documentTypes.toJS()).toEqual({ a: true });
    });
  });
});