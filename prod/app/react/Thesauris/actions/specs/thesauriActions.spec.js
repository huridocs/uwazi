...
var actions = _interopRequireWildcard(require("../thesauriActions"));
...
describe('thesaurisActions', () => {
  ...
  beforeEach(() => {
    ...
    _fetchMock.default.
    post(`${_config.APIURL}thesauris`, { body: JSON.stringify({ testBackendResult: 'ok' }) });
  });
  ...
  describe('importThesauri', () => {
    const mockSuperAgent = (url = `${_config.APIURL}thesauris`) => {
    ...
    it('should save thesaurus, import csv data and notify', done => {
      ...
      then(() => {
        expect(_superagent.default.post).toHaveBeenCalledWith(`${_config.APIURL}thesauris`);
  ...
  describe('addValue()', () => {
    it('should add an empty value to the thesauri', () => {
      ...
      expect(_reactReduxForm.actions.change).toHaveBeenCalledWith('thesauri.data.values', [{ label: 'something' }, { label: '', id: 'unique_id' }]);
    });
  });
  ...
  describe('addGroup()', () => {
    it('should add a new group at the end', () => {
      ...
      expect(_reactReduxForm.actions.change).
  ...
  describe('removeValue()', () => {
    it('should remove the value from the list', () => {
      ...
      expect(_reactReduxForm.actions.change).toHaveBeenCalledWith('thesauri.data.values', [
      { label: 'A', id: 2, values: [{ label: 'D', id: 3 }, { label: 'C', id: 4 }, { label: '' }] },
      { label: '' }]);
  ...
  describe('updateValues', () => {
    ...
    it('should set the updated values in the store', () => {
      ...
      expect(_reactReduxForm.actions.change).toHaveBeenCalledWith('thesauri.data.values', values);
    });
    ...
    it('should not remove a group from the root list', () => {
      ...
      actions.updateValues(values)(dispatch, getState);
      expect(_reactReduxForm.actions.change).not.toHaveBeenCalled();
    });
    ...
    describe('if there is a single empty item inside the list', () => {
      it('should move the empty item to the bottom of the list', () => {
        ...
        expect(_reactReduxForm.actions.change).toHaveBeenCalledWith('thesauri.data.values', expectedValues);
      });
    });
