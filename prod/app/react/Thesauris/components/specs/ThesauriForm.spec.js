...
var _ThesauriForm = require("../ThesauriForm.js");function _interopRequireDefault(obj) {return obj && obj.__esModule ? obj : { default: obj };}
...
describe('ThesauriForm', () => {
  ...
  beforeEach(() => {
    props = {
      ...
      thesauris: _immutable.default.fromJS([{ name: 'Countries' }]),
  ...
  const render = () => {
    component = (0, _enzyme.shallow)(_react.default.createElement(_ThesauriForm.ThesauriForm, props));
  ...
  describe('render', () => {
    it('should render DragAndDropContainer with thesauri items', () => {
      render();
      expect(component.find(_DragAndDrop.DragAndDropContainer).props().renderItem).toBe(component.instance().renderItem);
  ...
  describe('mapStateToProps', () => {
    ...
    it('should map the thesauri to initialValues', () => {
      expect((0, _ThesauriForm.mapStateToProps)(state).thesauri).toEqual({ name: 'thesauri name', values: [] });
      expect((0, _ThesauriForm.mapStateToProps)(state).thesauris).toEqual(_immutable.default.fromJS([{ name: 'Countries' }]));
    });
  });
  ...
  describe('validation', () => {
    describe('name duplicated', () => {
      ...
      function testValidationResult(inputValue, expectedResult) {
        const { name: { duplicated } } = _ThesauriForm.ThesauriForm.validation(thesauris, id);
