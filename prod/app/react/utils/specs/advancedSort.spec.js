"use strict";var _advancedSort = require("../advancedSort");

describe('Advanced Sort', () => {
  it('should sort regular characters (not affecting original)', () => {
    const baseArray = ['z', 'a', 'r', 'f'];

    expect((0, _advancedSort.advancedSort)(baseArray)).toEqual(['a', 'f', 'r', 'z']);
    expect((0, _advancedSort.advancedSort)(baseArray, { order: 'desc' })).toEqual(['z', 'r', 'f', 'a']);
  ...
  it('should sort special characters (not affecting original)', () => {
    ...
    expect((0, _advancedSort.advancedSort)(baseArray)).toEqual(['Á', 'é', 'f', 'R', 'z']);
  ...
  it('should sort strings of numbers correctly (not affecting original)', () => {
    ...
    expect((0, _advancedSort.advancedSort)(baseArray, { treatAs: 'number' })).toEqual(['1', '3', '3.1', '10', '200']);
    expect((0, _advancedSort.advancedSort)(baseArray, { treatAs: 'number', order: 'desc' })).toEqual(['200', '10', '3.1', '3', '1']);
  ...
  describe('Sorting objects', () => {
    ...
    it('should allow sorting by a single property (not affecting original)', () => {
      ...
      expect((0, _advancedSort.advancedSort)(baseArray, { property: 'b' })).toEqual([indexB, indexC, indexE, indexD, indexA]);
      expect(baseArray).toEqual([indexA, indexB, indexC, indexD, indexE]);
      expect((0, _advancedSort.advancedSort)(baseArray, { property: 'a' })).toEqual([indexA, indexB, indexC, indexD, indexE]);
      expect(baseArray).toEqual([indexA, indexB, indexC, indexD, indexE]);
    });
    ...
    it('should place items missing the property at the end', () => {
      ...
      expect((0, _advancedSort.advancedSort)(baseArray, { property: 'b' })).toEqual([indexC, indexE, indexD, indexA, indexB]);
      expect((0, _advancedSort.advancedSort)(baseArray, { property: 'b', order: 'desc' })).toEqual([indexA, indexD, indexE, indexC, indexB]);
      expect(baseArray).toEqual([indexA, indexB, indexC, indexD, indexE]);
    });
    ...
    it('should allow number sorting', () => {
      ...
      expect((0, _advancedSort.advancedSort)(baseArray, options)).toEqual([indexB, indexC, indexA, indexD, indexE]);
    ...
    it('should allow sorting by sub properties', () => {
      ...
      expect((0, _advancedSort.advancedSort)(baseArray, options).slice(0, 3)).toEqual([indexC, indexB, indexA]);
      expect((0, _advancedSort.advancedSort)(baseArray, options)[3] === indexD || (0, _advancedSort.advancedSort)(baseArray, options)[3] === indexE).toBe(true);
      expect((0, _advancedSort.advancedSort)(baseArray, options)[4] === indexD || (0, _advancedSort.advancedSort)(baseArray, options)[4] === indexE).toBe(true);
    });
  });
  ...
  describe('Sorting doted list numbers', () => {
    it('should sort list according to parameters passed', () => {
      ...
      expect((0, _advancedSort.advancedSort)(list, options)).toEqual(expectedResult);
    });
    ...
    it('should sort objects with dotted lists', () => {
      ...
      expect((0, _advancedSort.advancedSort)(list, options)).toEqual([{ a: '2.1' }, { a: '10' }]);
    });
  });
});