...
function fixRemainingSyntax(content) {
  ...
  // Pattern: const func = param: any => should be const func = (param: any) =>
  modifiedContent = modifiedContent.replace(
    /const\s+(\w+)\s*=\s*(\w+):\s*any\s*=>\s*{/g,
    (match, func, param) => {
      changed = true;
      return `const ${func} = (${param}: any) => {`;
    }
  );

  // Fix incorrect parameter type annotations in arrow function assignments
  ...
  for (const file of files) {
    try {
      ...
      const hasIssues = /\(\w+:\s*any\s*=>\s*{/.test(content) || 
                       ...
                       /const\s+\w+\s*=\s*\w+:\s*any\s*=>\s*{/.test(content);
      
      if (!hasIssues) continue;

      console.log(`🔧 Processing: ${file}`);
      
      const { content: modifiedContent, changed } = fixRemainingSyntax(content);
      
      if (changed) {
        fs.writeFileSync(file, modifiedContent);
        console.log(`  ✅ Fixed remaining syntax in ${file}`);
        filesModified++;
        totalReplacements++;
      }
    } catch (error) {
