...
describe('Collection', () => {
  ...
  it('should restore default library view', () => {
    cy.get('div[data-testid=settings-collection]').scrollTo('top');
    cy.get('#roles').select('Cards', { force: true });
    cy.get('#landing-page').clear({ force: true });
    cy.contains('button', 'Save').click();
  });
});
